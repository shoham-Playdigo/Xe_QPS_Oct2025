...
def main() -> int:
    ...
    if not DRY_RUN and not CONFIRM_LIVE:
        print("DRY_RUN=0 but CONFIRM_LIVE!='YES'. Refusing to run live updates.", file=sys.stderr)
        return 3

    print("Authenticating…")
    token = auth_token()
    print("Auth OK. DRY_RUN =", DRY_RUN)
    ...
    for s in summaries:
        ...
        # read detail for current_limit & real_qps
        try:
    ...
    if DRY_RUN:
        print("DRY_RUN: no live updates sent.")
    else:
