...
# --- START: EXCLUSION SETUP ADDITION ---
...
        srpm     = float(s.get("sRPM") or 0)
        ...
        # --- START: COMBINED EXCLUSION CHECK REPLACEMENT ---
        # Checks by ID (using the EXCLUDED_dsp_id variable) or by Name (using EXCLUDED_NAMES)
        is_excluded = False
        reason_text = ""
        
        if dsp_id in EXCLUDED_IDS:
            is_excluded = True
            reason_text = "excluded by ID"
        elif dsp_name.strip().lower() in EXCLUDED_NAMES:
            is_excluded = True
            reason_text = "excluded by Name"

        if is_excluded:
            print(f"Skipping DSP {dsp_name} (ID: {dsp_id}) - {reason_text}")
            results.append({
                "dsp_id": dsp_id,
                "name": dsp_name,
                "sRPM": srpm,
            ...
            continue
        # --- END: COMBINED EXCLUSION CHECK REPLACEMENT ---
